...
class SmuflExplorer(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('SMuFL Explorer Pro - Debug Mode')
        ...
        # Selezione automatica di un font musicale se presente
        for f in ["Bravura", "Finale Maestro", "Petaluma", "Leland"]:
        ...
        ctrl_layout.addWidget(self.font_combo, 1)
        ...
        self.cat_combo.addItems(SM_RANGES.keys())
        self.cat_combo.currentIndexChanged.connect(self.update_grid)
    ...
    def update_grid(self):
        ...
        start, end = SM_RANGES[category]
        
        display_font = QFont(font_name, self.current_zoom)
        # Otteniamo i dati grezzi del font per verificare se il glifo esiste davvero
        raw_font = QRawFont.fromFont(display_font)
        
        cols = max(1, self.width() // (self.current_zoom + 30))
        added_count = 0

        for code in range(start, end + 1):
            char = chr(code)
            
            # VERIFICA: Il font ha questo simbolo? 
            # Se raw_font.hasGlyph restituisce False, il font mostrerà un quadrato vuoto.
            if not raw_font.hasGlyph(code):
                continue # Salta i simboli non disegnati in questo font specifico

            btn = QPushButton(char)
            btn.setFont(display_font)
            btn.setFixedSize(self.current_zoom + 20, self.current_zoom + 20)
            btn.setToolTip(f"U+{code:04X}")
            btn.clicked.connect(lambda ch, c=char: self.copy_char(c))
            
            self.grid.addWidget(btn, added_count // cols, added_count % cols)
            added_count += 1

        self.container.setUpdatesEnabled(True)
        self.status.setText(f"Visualizzati {added_count} simboli validi per {font_name}")

    def copy_char(self, char):
        pyperclip.copy(char)
        self.status.setText(f"Copiato: {char} (U+{ord(char):04X})")

if __name__ == '__main__':
    app = QApplication(sys.argv)
    # Stile dark per visibilità
    app.setStyleSheet("QWidget { background: #222; color: #eee; } QPushButton { background: #333; border: 1px solid #555; }")
    win = SmuflExplorer()
    win.show()
    sys.exit(app.exec())
